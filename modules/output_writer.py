...
from modules import dedup_engine, identity, mapper
...
MANUAL_CHOICE_COL = _MANUAL_HEADER.index("בחר")  # אינדקס עמודת "בחר" (לתיבת-סימון ב-main)
...
# ערכי "מסומן" בעמודת "בחר": תיבת-סימון מחזירה TRUE; תאימות-לאחור להקלדה ידנית.
_CHECKED_VALUES = {"true", "✓", "v", "x", "1", "כן"}
_CHECK_TRUE = "TRUE"    # ערך מסומן (תיבת-סימון מסומנת)
_CHECK_FALSE = "FALSE"  # ערך לא-מסומן (תיבת-סימון ריקה)


def _is_checked(value: str) -> bool:
    """True אם תא 'בחר' מסומן (תיבת-סימון TRUE או סימון-יד). '' / 'FALSE' → לא-מסומן."""
    return str(value).strip().casefold() in _CHECKED_VALUES
...
def _cmp_value(value: object, field: str, digits_only: set[str]) -> str:
    """ערך מנורמל להשוואת-זהות — אותו נירמול כמו המנוע (digits-only לטלפון/ת"ז)."""
    if field in digits_only:
        return _NON_DIGITS.sub("", str(value if value is not None else ""))
    return identity.normalize(value)
...
def build_manual_grid(
    ...
    *,
    ...
    marked: {local_key → sf_id שכבר נבחר} — מסמן מחדש (TRUE) בשורת-המועמד התואמת, כדי
            לשמר את בחירות המשתמש בכתיבה-חוזרת (אידמפוטנטיות).
    digits_only_fields: שדות לנירמול ספרות-בלבד בהשוואת-הזהות (כמו המנוע).

    מחזיר (grid, cell_colors):
      grid:        list[list[str]] עם שורת-כותרת אחת; רק כותרת = אין רשומות לטיפול ידני.
                   עמודת "בחר" מכילה TRUE/FALSE (תיבת-סימון) למועמדים.
      cell_colors: ירוק על שדה זהה-למקור (מקור + מועמד), ורקע-בלוק מתחלף (band_a/band_b)
                   על עמודות-המטא להפרדה ויזואלית בין קבוצות.
    """
    marks = marked or {}
    digits_only = digits_only_fields or set()
    field_pairs = _field_columns(columns, object_api)
    fields = [api for api, _label in field_pairs]
    ...
    meta_cols = len(_MANUAL_HEADER)  # עמודות-המטא (0..meta_cols-1); שדות מ-meta_cols
    group_idx = 0
    ...
    for person in dedup_result.persons:
        ...
        band = "band_a" if group_idx % 2 == 0 else "band_b"

        # שורת המקור: ערכי רשומת-הקלט (ממוזגים אם כמה רשומות-חבר)
        merged = _consolidate(person.record_indices, record_values, fields)
        source_row_idx = len(grid)
        grid.append(
            [person.local_key, reason, src, "", _ROW_SOURCE, ""]
            + [merged[f] for f in fields]
        )

        # נירמול ערכי-המקור פעם אחת להשוואה מול כל מועמד
        norm_src = [_cmp_value(merged[f], f, digits_only) for f in fields]
        source_field_matched = [False] * len(fields)

        # שורה לכל מועמד-DB (ambiguous בלבד; ל-unkeyed אין מועמדים)
        for sid in person.match_ids:
            db_rec = db_by_id.get(sid, {})
            check = _CHECK_TRUE if marks.get(person.local_key) == sid else _CHECK_FALSE
            cand_row_idx = len(grid)
            grid.append(
                [person.local_key, "", "", check, _ROW_DB, sid]
                + [str(db_rec.get(f, "") or "").strip() for f in fields]
            )
            # ירוק על שדה-מועמד שזהה למקור (אותו נירמול כמו המנוע)
            for j, f in enumerate(fields):
                if norm_src[j] and _cmp_value(db_rec.get(f, ""), f, digits_only) == norm_src[j]:
                    cell_colors.append((cand_row_idx, meta_cols + j, "green"))
                    source_field_matched[j] = True

        # ירוק על שדה-המקור אם לפחות מועמד אחד התאים בו
        for j, matched in enumerate(source_field_matched):
            if matched:
                cell_colors.append((source_row_idx, meta_cols + j, "green"))

        # רקע-בלוק מתחלף על עמודות-המטא לכל שורות הקבוצה (מקור + מועמדים)
        for row0 in range(source_row_idx, len(grid)):
            for col0 in range(meta_cols):
                cell_colors.append((row0, col0, band))
...
def parse_manual_choices(manual_rows: list[list[str]]) -> tuple[dict[str, str], list[str]]:
    ...
    בחירה = שורת **מאגר** שעמודת "בחר" שלה מסומנת (תיבת-סימון TRUE, או סימון-יד) →
    ה-Id שלה נבחר ל-local_key. שורה ראשונה = כותרת (לאיתור אינדקסי העמודות לפי שם).
    מספר סימונים לאותו local_key → אזהרה, נלקח הראשון.
    """
    ...
    for row in manual_rows[1:]:
        if cell(row, c_type) != _ROW_DB or not _is_checked(cell(row, c_mark)):
            continue
